...
try:
    ...
    search_box.send_keys("S7 Airlines отзывы 2023")
    random_delay()
    ...
    for result in results:
        try:
            ...
            # Затем находим ссылку (a) внутри h2
            a_tag = h2_tag.find_element(By.CSS_SELECTOR, "a")

            # Извлекаем название (текст из span внутри a)
            title = a_tag.find_element(By.CSS_SELECTOR, "span").text

            # Извлекаем ссылку (href) из a
            ...
            print(f"Ошибка при извлечении данных из результата: {inner_e}")
